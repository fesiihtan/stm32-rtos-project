 ...
 * @date      : June 1, 2026
 *
 *  Contains the function definitions and the task logic for the error handler
 */
...
/**
 * Blinks the user led to indicate an error event.
 */
static void error_handler_led_blink()
{
  if(!is_blinking_active)
  {
    is_blinking_active = true;
    for(uint8_t i = 0; i < EVT_BLINK_NUMBER; i++)
    {
    ...
    // Leave the LED on after blinking to indicate an error state
