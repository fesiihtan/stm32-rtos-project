 ...
 *      Author: Fesih TAN
 *      @brief: Contains the implementation of the Sensors Task, which is responsible for acquiring data
...
/**
 * Queue handle for the Sensor Task
 */
...
{
  // Check if there are any message in the queue without actually reading them
...
static void sensor_task(void *param)
{
  ...
  sensor_task_data_t sensors_task_data = {0};

  // Initilaizes the sensors and let the Error Handler know if there was an issue
  ...
  // Todo: Get the sampling time interval from the Modbus Holding Registers and update the xFrequency variable
  // For now, just set it to 1000ms
  ...
  while(1)
  {
     ...
     *       2) If sensor read succesful:
     *          Check if VOC sensor is warmed up (value not 0), toggle LED, update sensor task data structure, update Modbus Data Manager Register
...
void sensors_task_start(void)
{
  ...
  configASSERT(pdPASS == xTaskCreate(sensor_task,
                                     "Sensors Task",
