 ...
 *      Author: Fesih TAN
 *      @brief: Contains the initialization and configuration of various timers used in the application
 */
#include "tim.h"
...
void tim2_init(void)
{
  ...
  // Trigger frequency = 10,000 / (1 + ARR) = 10,000 Hz /(1 + 9999) = 1 Hz
  TIM2->PSC = 8999;
  TIM2->ARR = 9999;
...
void tim5_init(void)
{
  ...
  // Force an update event to apply prescaler immediatly
  TIM5->EGR |= (TIM_EGR_UG);
