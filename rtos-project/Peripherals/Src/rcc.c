 ...
 * @author    : Fesih TAN
 * @date      : May 28, 2026
 * @brief     : Contains function definitions required for RCC initialization and validation.
 */
 ...
 * @param mcu_freq_mhz: Desired system clock frequency in MHz
 * @return Optimal PLLP value (2, 4, 6, or 8)
 */
...
/**
 *  @brief Configures the PLLM, PLLN and PLLP values for the PLL. PLLM is set based on the predefined HSE frequency
 *         PLLN and PLLP are calculated to achieve the desired system clock freqeuncy based on the PLL input frequency.
 *
...
static void rcc_pll_config(uint8_t sysclk_freq_mhz)
{
  ...
  // Set PLLM
  // For the PLLM parameter, use the RCC_HSE_MHZ definitions
  ...
  // Set the PLLP
  // To calculate corresponding PLLP bit values, use the following mapping:
  // PLLP = 2 -> PLLP bits = 00, PLLP = 4 -> PLLP bits = 01 etc.
  PLLP = (PLLP / 2) - 1; // Convert PLLP value to corresponding bit
...
{
  // RCC Initialization:
  // Set the system frequency, configure wait states,
  ...
  rcc_pll_source(RCC_PLL_SRC_HSE); // Set PLL source as HSE

  rcc_pll_config(sysclk_freq_mhz); // Configure PLLM, PLLN and PLLP values based on the desired system clock frequency

  rcc_pll_enable(); // Enable PLL and wait until ready

  rcc_ahb_set_prescaler(RCC_SYSCLK_DIV_1); // Set AHB prescaler to 1 (no division)

  rcc_sysclk_set_source(RCC_SYSCLK_SRC_PLLP); // Set PLL as the system clock source

  while(RCC_CFGR_SWS_PLL != rcc_sysclk_get_source()); // Wait until PLL is used as the system clock source

  rcc_hsi_disable(); // Disable HSI to save power

  rcc_apb1_set_prescaler(RCC_APB1_DIV_4); // Set APB1 prescaler to 4 (max 50 MHz)
  rcc_apb2_set_prescaler(RCC_APB2_DIV_4); // Set APB2 prescaler to 4 (max 100 MHz)
